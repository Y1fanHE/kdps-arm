    ...
    def pre(self, id: str, args, kwargs, obj=None):
        ...
        if generation % self.every_n_steps == 0:
            print(" | ".join([
                ...
                # add the following lines and reinstall the library to enable
                # the function that compute and log gene rate of in-solution
                # knowledge in the entire population
                "VIRUSES: gene_rate={gn_rate:.3f}".format(
                    gn_rate=search.population.virus_gene_rate()
                )
            ]))
