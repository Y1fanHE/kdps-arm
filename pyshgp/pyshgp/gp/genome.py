...
class VirusSpawner:
    def __init__(self, viruses: Sequence[Genome] = [Genome()]):
        self.viruses = viruses
        self.size = len(self.viruses)

    def spawn_virus(self, index=None, p=None, return_index=False):
        if index == None:
            if p == None:
                idx = np.random.choice(self.size)
            else:
                idx = np.random.choice(self.size, p=p)
        else:
            idx = index
