...
class GeneticAlgorithm(SearchAlgorithm):
    ...
    def _make_child(self) -> Individual:
        ...
        child_genome = op.produce(parent_genomes, self.config.spawner, self.config.max_genome_size)
...
class VirusGeneticAlgorithm(GeneticAlgorithm):

    def __init__(self, config: SearchConfiguration, virus_spawner):
        super().__init__(config)
        self.virus_spawner = virus_spawner
    ...
    def _make_child(self) -> Individual:
        ...
        parent_genomes = [p.genome for p in selector.select(self.population, n=op.num_parents)]
        child_genome = op.produce(parent_genomes, self.config.spawner, self.config.max_genome_size,
                                  virus_spawner=self.virus_spawner)
        return Individual(child_genome, self.config.signature)


class AdaptiveVirusGeneticAlgorithm(SearchAlgorithm):

    def __init__(self, config: SearchConfiguration, virus_spawner):
        super().__init__(config)
        self.virus_spawner = virus_spawner
        self.success_virus_idx = []

    def _make_child(self) -> Individual:
        op = self.config.get_variation_op()
        selector = self.config.get_selector()
        # select parents
        parents = selector.select(self.population, n=op.num_parents)
        # get parent genomes
        parent_genomes = [p.genome for p in parents]
        # get parent error vector
        parent_error_vectors = [p._error_vector for p in parents]
        # get virus and index
        virus_from_success_index = False
        if len(self.success_virus_idx) <= 0:
            virus, idx = self.virus_spawner.spawn_virus(return_index=True)
        else:
            if np.random.random() < 0.5:
                virus, idx = self.virus_spawner.spawn_virus(return_index=True)
            else:
                virus_from_success_index = True
                idx = np.random.choice(self.success_virus_idx)
                virus, idx = self.virus_spawner.spawn_virus(index=idx, return_index=True)

        # rand is a random number to control the method of variation
        # if rand >= 0.5, perform add and del mutation only
        # if rand < 0.5, perform replace mutation only
        rand = np.random.random()
        child_genome = op.produce(parent_genomes, self.config.spawner, self.config.max_genome_size,
                                  virus=virus, rand=rand, do_mutation=rand>=0.5)
        # when virus is used in replace mutation, pass it to the child individual
        return Individual(child_genome, self.config.signature,
                          parent_error_vector=parent_error_vectors[0],
                          virus_index=idx if rand < 0.5 else None,
                          virus_from_success_index=virus_from_success_index)

    @tap
    def step(self):
        for p in self.population:
            if p.is_improved and p.virus_index != None:
                self.success_virus_idx.append(p.virus_index)
        super().step()
        self.population = Population(
            [self._make_child() for _ in range(self.config.population_size)]
        )
        indexes = []
        for p in self.population:
            if p.virus_index != None and p.virus_from_success_index:
                indexes.append(str(p.virus_index))
        with open("vdx.csv", "a") as vdx_file:
            vdx_file.write(",".join(indexes)+"\n")
...
def get_search_algo(name: str, **kwargs) -> SearchAlgorithm:
    ...
    name_to_cls = {
        ...
        "VGA": VirusGeneticAlgorithm,
        "AVGA": AdaptiveVirusGeneticAlgorithm,
