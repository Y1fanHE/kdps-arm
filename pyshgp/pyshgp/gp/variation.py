...
from pyshgp.gp.genome import Genome, GeneSpawner, VirusSpawner
...
class VariationOperator(ABC):
    ...
    @tap
    @abstractmethod
    def _produce(self, parents: Sequence[Genome], spawner: GeneSpawner, **kwargs) -> Genome:
    ...
    def produce(self, parents: Sequence[Genome], spawner: GeneSpawner, max_genome_size: Optional[int] = None, **kwargs) -> Genome:
        if max_genome_size == None:
            return self._produce(parents, spawner, **kwargs)
        return self._truncate(self._produce(parents, spawner, **kwargs), max_genome_size)
...
class VariationPipeline(VariationOperator):
    ...
    def _produce(self, parents: Sequence[Genome], spawner: GeneSpawner, **kwargs) -> Genome:
        ...
        """
        super()._produce(parents, spawner, **kwargs)
        self.checknum_parents(parents)
        child = parents[0]
        for op in self.operators:
            child = op._produce([child] + parents[1:], spawner, **kwargs)
        return child
...
class LiteralMutation(VariationOperator, ABC):
    ...
    @tap
    def _produce(self, parents: Sequence[Genome], spawner: GeneSpawner = None, **kwargs) -> Genome:
        """Produce a child Genome from parent Genomes and optional GenomeSpawner.
...
class DeletionMutation(VariationOperator):
    ...
    @tap
    def _produce(self, parents: Sequence[Genome], spawner: GeneSpawner,
                 do_mutation: bool = True, **kwargs) -> Genome:
        """Produce a child Genome from parent Genomes and optional GenomeSpawner.
...
class InsertionMutation(VariationOperator):

    def __init__(self, insertion_rate: float = 0.01):
        super().__init__(1)
        self.rate = insertion_rate

    @tap
    def _produce(self, parents: Sequence[Genome], spawner: GeneSpawner,
                 virus: Genome = None, virus_spawner: VirusSpawner = None,
                 rand: float = None, **kwargs) -> Genome:
        super()._produce(parents, spawner, **kwargs)
        self.checknum_parents(parents)
        if virus == None:
            inserted_virus = virus_spawner.spawn_virus()
        else:
            inserted_virus = virus
        new_genome = Genome()
        insert_point = choice(len(parents[0]))
        for gene in parents[0][:insert_point]:
            new_genome = new_genome.append(gene)
        ########## Insert virus belowe ##########
        if rand == None:
            rand = random()
        if rand < self.rate:
            for gene in inserted_virus:
                new_genome = new_genome.append(gene)
        ########## Insert virus above  ##########
        for gene in parents[0][insert_point:]:
            new_genome = new_genome.append(gene)
...
class ReplaceMutation(VariationOperator):

    def __init__(self, replace_rate: float = 0.01):
        super().__init__(1)
    ...
    def _produce(self, parents: Sequence[Genome], spawner: GeneSpawner,
        ...
        self.checknum_parents(parents)
        if virus == None:
            replaced_virus = virus_spawner.spawn_virus()
        elif virus == Genome():
            return parents[0]
        else:
            replaced_virus = virus
        if len(replaced_virus) >= len(parents[0]):
            return replaced_virus
        new_genome = Genome()
        replace_point = choice(len(parents[0])-len(replaced_virus))
        for gene in parents[0][:replace_point]:
            new_genome = new_genome.append(gene)
        ########## Insert virus belowe ##########
        if rand == None:
            rand = random()
        if rand < self.rate:
            for gene in replaced_virus:
                new_genome = new_genome.append(gene)
        ########## Insert virus above  ##########
        for gene in parents[0][replace_point+len(replaced_virus):]:
            new_genome = new_genome.append(gene)
        return new_genome
